...
import { deepEqual, infoIconDefaultClass, tooltipDefaultCSS } from 'projects/shared/utils/various';
...
export class NomSelectComponent extends BaseField implements OnInit, OnDestroy {
    ...
    if (this.showAllGroup) {
      ...
      this.ngSelectComponent.itemsList.mapSelectedItems = () => {
        ...
        if (_this._ngSelect.hideSelected) {
          for (let selectItem of _this.selectedItems) {
      ...
      switchMap((term) => {
        ...
        return this.slice$.pipe(
          exhaustMap(([start, end]) => {
            ...
            return this.nomService.getNomsByTerm({ term, offset, limit }, this.filter).pipe(
              map((noms) => {
  ...
  writeValue(value: any): void {
    if ((this.multiple && value && (value as []).length) || (!this.multiple && value)) {
      if (this.multiple && this.showAllGroup && value.includes(ALL_GROUP_ID)) {
        this.nomService
          .getNomsByTerm({ term: '', offset: 0, limit: this.sliceSize }, this.filter)
          .pipe(
            tap((noms) => {
              this.items = noms;
            }),
      ...
      this.nomService
        .getNomsById({ ids: this.multiple ? value : [value] }, this.filter)
        .pipe(
          tap((noms) => {
            this.items = noms;
